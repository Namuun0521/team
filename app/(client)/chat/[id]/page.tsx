...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const channel = client.channels.get(`chat:${id}`);
    ...
    // Cleanup - connection state шалгаж хаана
    return () => {
      isMounted = false;
      ...
      try {
        ...
        // Аль хэдийн хаагдсан бол алгасна
      }
  ...
  const send = useCallback(async () => {
    ...
    try {
      ...
      setMessages((prev) =>
        prev.map((m) => (m === optimisticMsg ? newMsg : m)),
      );
    } catch (e) {
      console.error(e);
      setMessages((prev) => prev.filter((m) => m !== optimisticMsg));
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1">
              ...
              <span className="text-xs font-medium text-slate-600">
                {connected ? "Live" : "Холбогдож байна..."}
              </span>
            </div>
